...
p.add_argument('--model', type=str, default='gelu', required=False, choices=['sine', 'tanh', 'sigmoid', 'relu'],
...
p.add_argument('--counter_end', type=int, default=50000, required=False, help='Defines the linear step for curriculum training starting from the initial time')
p.add_argument('--num_src_samples', type=int, default=10000, required=False, help='Number of source samples at each time step')


p.add_argument('--clip_grad', default=False, type=float, help='Clip gradient.')
...
dataset = dataio.SwarmHJI_2d_new(numpoints=65000, alpha=opt.alpha, u_max=1, u_min=0, t_min=opt.tMin, t_max=opt.tMax,
...
model = modules.SingleBVPNet(in_features=5, out_features=2, type=opt.model, mode=opt.mode,
                             final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)

model.to(device)

loss_fn = loss_functions.initialize_swarm_hji_2d_new(dataset)

root_path = os.path.join(opt.logging_root, 'swarm_hji-2d_2.5_general_test_gelu/')
